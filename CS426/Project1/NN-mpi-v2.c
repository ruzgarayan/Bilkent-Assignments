...
void writeOutput(double *output, int columns) {
	...
	file = fopen("NN-mpi-v2-output.txt", "w");
	if (file == 0)
		return ;
...
int main(int argc, char **argv) 
{
	...
	if (numprocs < 2) {
		if (myid == 0) printf("For this program there must be at least 1 master and 1 worker.\n So call with at least -np 2.\n");
		MPI_Finalize(); 
		return 0;
	}
	...
	if (myid == 0)
	{
		...
		//Find out how many columns will be given to each worker and communicate this information
