...
{ 
	...
	if (myid == 0)
	{
		...
		//Collect the results from the other processors and add up
		int resultsReceived[4];
		for (int i = 1; i < numprocs; i++) {
			MPI_Recv (resultsReceived, 4, MPI_INT, i, 2, MPI_COMM_WORLD, &s);
			for (int j = 0; j < 4; j++)
				results[j] += resultsReceived[j];
		}
		end = MPI_Wtime();
		...
		//Send the results to the processor 0 to accumulate there
